...
bool SystemClass::Initialize()
{
	...
	if(!result)
	{
		MessageBox(m_hwnd, (LPCSTR)"Could not initialize the window.", (LPCSTR)"Error", MB_OK);
		return false;
	}
...
bool SystemClass::InitializeWindows(OpenGLClass* OpenGL, int& screenWidth, int& screenHeight)
{
	...
	if(!result)
	{
		MessageBox(m_hwnd, (LPCSTR)"Could not initialize OpenGL, check if video card supports OpenGL 4.0.", (LPCSTR)"Error", MB_OK);
		return false;
	}
