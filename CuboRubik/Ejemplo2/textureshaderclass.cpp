...
bool TextureShaderClass::SetShaderParameters(OpenGLClass* OpenGL, float* worldMatrix, float* viewMatrix, 
	float* projectionMatrix, int textureUnit, int otro)
{
	...
	// Set the texture in the pixel shader to use the data from the first texture unit.
	location = OpenGL->glGetUniformLocation(m_shaderProgram, "otrox");
	if (location == -1)
	{
		return false;
	}
	OpenGL->glUniform1i(location, otro);
